...
export const productsApi = {
  ...
  getByPrintMethod: async (printMethod: string): Promise<PaginatedResponse<Product>> => {
    const { data } = await api.get(`/api/storefront/products/print-method/${printMethod}`);
    return data;
  },

  getByCategory: async (category: string): Promise<PaginatedResponse<Product>> => {
    const { data } = await api.get(`/api/storefront/products/category/${category}`);
    return data;
  },
...
export const cartApi = {
  ...
  add: async (productId: string, quantity: number, variantId?: string): Promise<Cart> => {
  ...
  update: async (itemId: string, quantity: number): Promise<Cart> => {
    const { data } = await api.put(`/api/storefront/cart/update/${itemId}`, { quantity });
    return data;
  },
...
export const customerApi = {
  ...
  addAddress: async (address: Omit<CustomerAddress, 'id' | 'customer_id'>): Promise<CustomerAddress> => {
    const { data } = await api.post('/api/storefront/customers/me/addresses', address);
    return data;
  },

  updateAddress: async (id: string, address: Partial<CustomerAddress>): Promise<CustomerAddress> => {
    const { data } = await api.put(`/api/storefront/customers/me/addresses/${id}`, address);
    return data;
  },
...
export const ordersApi = {
  ...
  getByNumber: async (orderNumber: string): Promise<Order> => {
    const { data } = await api.get(`/api/storefront/orders/number/${orderNumber}`);
    return data;
  },
  ...
  downloadDigital: async (orderId: string, itemId: string): Promise<Blob> => {
    const { data } = await api.get(`/api/storefront/orders/${orderId}/items/${itemId}/download`, {
      responseType: 'blob',
    });
    return data;
  },
...
export const checkoutApi = {
  ...
  initialize: async (checkoutData: CheckoutInit): Promise<CheckoutResponse> => {
    const { data } = await api.post('/api/storefront/checkout/init', checkoutData);
    return data;
  },

  createPaymentIntent: async (amount: number, currency = 'usd'): Promise<PaymentIntentResponse> => {
    const { data } = await api.post('/api/storefront/checkout/create-payment-intent', {
      amount,
      currency,
    });
    return data;
  ...
  complete: async (params: {
    ...
    const { data } = await api.post('/api/storefront/checkout/complete', params);
    return data;
  },
};
