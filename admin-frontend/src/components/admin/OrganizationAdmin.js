  ...
  const handleStatusToggle = async (orgId, currentStatus) => {
    try {
      ...
      setOrganizations(prev =>
        prev.map(org => (org.id === orgId ? { ...org, settings: { ...org.settings, is_active: !currentStatus } } : org))
      );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-900">Organizations ({filteredOrganizations.length})</h3>
        </div>
                    ...
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          org.settings.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}
                      >
