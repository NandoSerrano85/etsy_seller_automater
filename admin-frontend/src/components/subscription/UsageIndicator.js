...
const UsageIndicator = ({
  ...
  // For non-mockup features or paid tiers without limits, just show totals
  const showLimitBar = feature === FEATURES.MOCKUP_GENERATOR && isFreeTier;

  const used = currentUsage.mockupsThisMonth;
  const total = mockupLimit;
  const remaining = remainingMockups;
  const percentage = total > 0 ? (used / total) * 100 : 0;
  const totalMockups = currentUsage.totalMockups || 0;
  const totalMockupImages = currentUsage.totalMockupImages || 0;
        ...
        <div className="flex items-center space-x-3">
          ...
          {showTotals && (
            <span className="text-gray-500">
              {totalMockups} sets • {totalMockupImages} images
            </span>
          )}
        </div>
      </div>
  ...
  return (
    <div className={`bg-white rounded-lg border p-4 ${className}`}>
      {/* Total counts - always show */}
      {showTotals && (
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-gray-900">{totalMockups}</div>
            <div className="text-xs text-gray-500">Mockup Sets</div>
          </div>
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-gray-900">{totalMockupImages}</div>
            <div className="text-xs text-gray-500">Mockup Images</div>
          </div>
        </div>
      )}

      {/* Monthly limit bar - only for free tier */}
      {showLimitBar && (
        <>
