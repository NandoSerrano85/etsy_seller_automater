...
def get_resizing_configs_from_db(shop_name: str):
    """Get canvas and size configurations from database for a specific shop."""
    configs = get_user_resizing_configs(shop_name)
    if configs:
        return configs['CANVAS'], configs['SIZING']
    else:
        # Fallback to hardcoded defaults if no database configs found
...
"""
def fit_image_to_center_canvas(resized_img, new_width_px, new_height_px, target_dpi, image_type, image_size=None, shop_name=None):
    # Get canvas configs from database or fallback to defaults
    if shop_name:
        CANVAS, _ = get_resizing_configs_from_db(shop_name)
    else:
...
"""
def resize_image_by_inches(image_path, image_type, image_size=None, image=None, target_dpi=STD_DPI, is_new_mk=False, shop_name=None):
    # Get sizing configs from database or fallback to defaults
    if shop_name:
        _, SIZING = get_resizing_configs_from_db(shop_name)
    else:
    ...
    if image_type == 'UVDTF Decal' or image_type == 'UVDTF Bookmark' or image_type == 'UVDTF Lid' or image_type == 'Custom 2x2' or image_type == 'UVDTF Ornament' or image_type == 'UVDTF Logo Cup Care Decal' or image_type == 'UVDTF Logo Bottom Shot Decal' or image_type == 'UVDTF Logo Sticker':
        return fit_image_to_center_canvas(resized_img, new_width_px, new_height_px, target_dpi, image_type, shop_name=shop_name)
        ...
        if (new_width_px > new_height_px) and is_new_mk:
            ...
            return fit_image_to_center_canvas(rotated_img, rotated_img.shape[1], rotated_img.shape[0], target_dpi, image_type, shop_name=shop_name)
        else:
            return fit_image_to_center_canvas(resized_img, new_width_px, new_height_px, target_dpi, image_type, shop_name=shop_name)
