...
const TemplatesTab = () => {
  ...
  const handleReauthorizeShopify = async () => {
    try {
      ...
      console.error('Error initiating re-authorization:', error);
      setMessage('Failed to initiate Shopify re-authorization');
    }
  };
...
const TemplateEditModal = ({ template, templateType, onSave, onClose }) => {
  ...
  const [themePermissionWarning, setThemePermissionWarning] = useState(null);
    ...
    const fetchShopifyMetadata = async () => {
      if (templateType === 'shopify' && selectedStore) {
        ...
        try {
          ...
          // Check for theme permission warning
          if (themesData.warning) {
            setThemePermissionWarning(themesData.warning);
          } else {
            setThemePermissionWarning(null);
          }
        } catch (err) {
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
          <div className="flex items-start justify-between">
            <div className="flex items-start">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800">Additional Permissions Required</p>
                <p className="text-sm text-amber-700 mt-1">{themePermissionWarning}</p>
              </div>
            </div>
            <button
              type="button"
              onClick={onReauthorize}
              className="ml-4 px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors whitespace-nowrap"
            >
              Grant Permissions
            </button>
          </div>
