...
const ListingsTab = ({ isConnected, authUrl, loading: parentLoading, error: parentError, onRefresh }) => {
  const api = useApi();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Manage your Etsy listings • {listings.length} total listings</p>
        </div>
      ...
      {listings.length === 0 ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between bg-gray-50 rounded-lg p-4">
            ...
            {selectedListings.length > 0 && (
              <button onClick={() => setSelectedListings([])} className="text-sm text-gray-500 hover:text-gray-700">
                Clear selection
              </button>
            )}
          </div>
      ...
      {showEditModal && editingListing && (
        <EditListingModal listing={editingListing} onClose={handleModalClose} onSuccess={handleEditSuccess} />
      )}

      {showBulkEditModal && (
        <BulkEditModal selectedListingIds={selectedListings} onClose={handleModalClose} onSuccess={handleEditSuccess} />
      )}
    </div>
  );
};
