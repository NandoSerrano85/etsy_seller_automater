...
export const useCachedApi = () => {
  ...
  const makeCachedApiCall = useCallback(async (
    cacheKey,
    apiCall,
    forceRefresh = false
  ) => {
  ...
  const fetchTopSellers = useCallback(async (forceRefresh = false, year = new Date().getFullYear()) => {
    if (!isEtsyConnected) {
      throw new Error('Etsy not connected');
    }
    
    return makeCachedApiCall('topSellers', async () => {
  ...
  const fetchMonthlyAnalytics = useCallback(async (forceRefresh = false, year = new Date().getFullYear()) => {
    if (!isEtsyConnected) {
      throw new Error('Etsy not connected');
    }
    
    return makeCachedApiCall('monthlyAnalytics', async () => {
      // Auth service will automatically handle token refresh and injection
      const response = await makeAuthenticatedRequest(`/dashboard/analytics?year=${year}`);
      
      return response?.data?.analytics || response || {
        monthly_revenue: [],
        monthly_orders: [],
        monthly_views: []
      };
  ...
  const fetchOrders = useCallback(async (forceRefresh = false) => {
    ...
    return makeCachedApiCall('orders', async () => {
      // Auth service will automatically handle token refresh and injection
  ...
  const fetchMockups = useCallback(async (forceRefresh = false) => {
    return makeCachedApiCall('mockups', async () => {
  ...
  const fetchAllDashboardData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const stats = {
        totalSales: monthlyAnalytics.status === 'fulfilled' ? 
          (monthlyAnalytics.value?.total_revenue || monthlyAnalytics.value?.data?.total_revenue || 0) : 0,
        totalOrders: orders.status === 'fulfilled' ? 
          (orders.value?.length || 0) : 0,
        totalDesigns: designs.status === 'fulfilled' ? 
          (designs.value?.length || 0) : 0,
        totalMockups: mockups.status === 'fulfilled' ? 
          (mockups.value?.length || 0) : 0
      };
  ...
  const refreshData = useCallback((dataType = null) => {
    if (dataType) {
      clearCache(dataType);
    } else {
      clearCache();
