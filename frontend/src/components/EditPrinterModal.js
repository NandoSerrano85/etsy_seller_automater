...
const EditPrinterModal = ({ isOpen, printer, onClose, onSubmit }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const result = await printerService.updatePrinter(printer.id, updateData);

      if (result.success) {
        updatePrinter(result.data);
        addNotification({
          type: 'success',
        ...
        addNotification({
          type: 'error',
          message: `Failed to update printer: ${result.error}`,
        });
      }
    } catch (error) {
      addNotification({
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-sage-900">Edit Printer</h2>
              <button
                onClick={handleClose}
