...
const useAuthStore = create(
  persist(
    (set, get) => ({
      ...
      setEtsyConnection: (userInfo, shopInfo) => set({
        ...
        return {
          ...
          etsyAuth: {
            ...
            accessTokenPreview: state.etsyTokens.accessToken ? 
              state.etsyTokens.accessToken.substring(0, 20) + '...' : 'None',
            refreshTokenPreview: state.etsyTokens.refreshToken ?
              state.etsyTokens.refreshToken.substring(0, 20) + '...' : 'None',
          ...
          timestamps: {
            ...
            tokenExpiresIn: state.etsyTokens.expiresAt ? 
              Math.max(0, Math.floor((state.etsyTokens.expiresAt - Date.now()) / 1000 / 60)) + ' minutes' : 'N/A'
          }
        };
      }
    }),
